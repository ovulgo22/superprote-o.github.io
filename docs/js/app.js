document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadContent = async (path) => {
        try {
            ...
            Prism.highlightAllUnder(mainContent); // V2.0: Destaque de sintaxe
            updateActiveLink(path);
            generateAndObserveToc(); // V2.0: Sumário com scroll spy
            mainContent.scrollTop = 0;
            document.title = findTitleByPath(path) || 'Docs 2.0';
        } catch (error) {
     ...
     */
    const handleRouting = () => {
        const defaultPath = navStructure[0]?.path || 'docs/home.md';
        const path = window.location.hash.substring(1) || defaultPath;
        loadContent(path);
    };

    /**
     * V2.0: Constrói a navegação da barra lateral a partir do _nav.json.
     * @param {Array} items - Array de itens de navegação.
     * @param {HTMLElement} parentElement - O elemento pai onde a navegação será construída.
     */
    const buildNav = (items, parentElement) => {
        ...
        items.forEach(item => {
            ...
            if (item.path) {
                ...
                li.textContent = item.title; // Para títulos de seção
    ...
    /**
     * V2.0: Gera o sumário ("Nesta Página") e configura o observador de rolagem.
     */
    ...
    /**
     * V2.0: Constrói o índice de pesquisa buscando todos os arquivos .md.
     * @param {Array} items - Array de itens de navegação.
     */
    ...
    /**
     * V2.0: Executa a pesquisa e exibe os resultados.
     */
    const performSearch = () => {
        ...
        if (results.length > 0) {
            results.forEach(item => {
                ...
                resultItem.onclick = () => searchResults.classList.remove('visible');
    ...
    const init = async () => {
        ...
        try {
            ...
            // Construir índice de pesquisa em segundo plano
            await buildSearchIndex(navStructure);
        } catch(error) {
            console.error("Falha ao carregar a estrutura de navegação:", error);
            mainContent.innerHTML = "<h1>Erro Crítico</h1><p>Não foi possível carregar a navegação do site (_nav.json).</p>";
        }
